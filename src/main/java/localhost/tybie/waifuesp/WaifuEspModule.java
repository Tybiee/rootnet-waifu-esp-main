...
/**
 * Main Waifu ESP module class
 */
@RootnetModule(name = "WaifuESP")
public final class WaifuEspModule extends AddonModule {
    ...
    private final ResourceLocation waifu;
    private final double waifuRatio;

    /**
     * Creates the module, downloading the cute anime girl image
     * @throws IOException If the file image could not be downloaded
     */
    WaifuEspModule() throws IOException {
        final BufferedImage image = ImageIO.read(new URL(WaifuEspAddon.WAIFU_URL));
        waifuRatio = ((double) image.getWidth()) / ((double) image.getHeight());
    ...
    /**
     * Renders the cute anime girls over players
     */
    @Override
    public void renderOverlay() {
        ...
        for (final Entity entity : MC.world.loadedEntityList) {
            if (entity instanceof AbstractClientPlayer && !entity.equals(renderViewEntity)) {
                renderWaifu((AbstractClientPlayer) entity, cameraPos, screenSize, partialTicks);
            }
        }
    }

    private void renderWaifu(final AbstractClientPlayer player, final Vec3d cameraPos, final ScreenSize screenSize,
        ...
        final double doubleWidth = doubleHeight * waifuRatio;
        final int width = (int) (doubleHeight * waifuRatio);
